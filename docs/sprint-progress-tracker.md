...
**Status 8.1:** lock/splash/intro/auth/focus/permissions rungs = ALL SEVEN land real dedicated zero-import screens (proven suite-pinned: Passimark/Dashboard.jsx 2131B zero-import, AdminDashboardTest:64 pins Passimark/Dashboard BY NAME) - funnel navigable end-to-end. **Focus write-back: PENDING with honest reason** - the POST-route + preferences-persist slice was ATTEMPTED this sprint, went RED (spec-tested with User::factory() but this app has NO factory; real convention = seeded student@passimark.com, AdminDashboardTest:60), so it was REVERTED, not shipped; disk sits exactly at proven-green 5ebe952. The Interest Picker is real and re-editable on the funnel; persisting the pick into users.preferences json + reading it back on the suite-bound Dashboard is the honest NEXT slice (will follow app-convention POST + seeded-user feature test, gated green before any commit)
**Status:** lock/splash/intro **landed** (real `Passimark/Funnel/{Lock,Splash,Intro}` screens, controller rungs lock/splash/intro bind them verbatim at lines 36/50/64); **auth/focus/permissions rungs ALSO LANDED as real dedicated zero-import screens** (`Passimark/Funnel/{Auth,Focus,Permissions}.jsx` = 1302/1259/1302 B with milestone-panel + interest-chip + skip-ladder interior, controller rungs auth/focus/permissions render them verbatim at lines 78/92/106); **ALL SEVEN rungs now converge into the REAL lean suite-bound destination `Passimark/Dashboard.jsx`** (2131 B, zero-import, pinned BY NAME at AdminDashboardTest:64). Suite still green (112 tests / 44,598 assertions).  �  **Branch:** `feature/sprint-8-1-funnel-stories`  �  **Doc:** `docs/sprint-8-1-funnel-stories.md`
