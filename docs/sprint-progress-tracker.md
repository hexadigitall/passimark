...
**Status:** Sprint 5 **done**, Sprint 6 **done**, Sprint 7 **done**, Sprint 8 **done**
...
- **Sprint 8 — certificates + QR verification — done** (see the Sprint 8 section below): `CertificateIssuer` mints deterministic `PMK-…` credentials on a passed/approved final, owner-only certificate screen with QR code, public `/verify/{credentialId}` page, Result/Track/Profile CTAs; manifest background already pinned at `#0F172A` in 7.5e (regression test added)
...
- **Answer-key backfill:** `seedPool()` now writes `correct_key`; migration `000013` backfills legacy CISSP rows from the flagged option. All **17,750** questions now carry `correct_key` (0 null, 0 mismatch vs the flagged option).
...
**Status 8.1 (CLOSED):** all seven rungs land real dedicated screens, the ladder is walked end to end, and the focus write-back is **SHIPPED** (commit `05d73d9`). Both pending defects from the previous revision are resolved:
- **The post-login rungs were orphaned.** `AuthController::login` returned `redirect()->intended(route('dashboard'))`, so signing in jumped straight to the catalog and rungs 5/6 were never shown to anyone (grep for `funnel/focus` outside the controller: zero hits). Fixed with `AuthController::landingFor()` — a learner who has not completed the post-login rungs lands on `passimark.funnel.focus`; once `permissions` is done, returning learners go straight to the dashboard. Staff never enter the learner funnel. Applies to login AND register.
- **Focus write-back landed** using the real convention (seeded `student@passimark.com`, no `UserFactory` exists). `saveFocus()` validates the pick with `Rule::in` against **live active `PassimarkCertificationTrack` rows** (205 certs / 9 regions, so a retired cert cannot be selected) and persists it into the `users.preferences` json column; `completeFunnel()` sets `preferences.funnel_completed`, which is what stops the learner being re-bounced into the ladder on every later login; `PassimarkCatalogController::focusPreference()` resolves the saved key back to a catalog row (title, region, done/total, percent) and the Dashboard renders a **"Your focus"** card with a Change link. `focus` is `null` when nothing was chosen.
- **Earlier in the same pass:** `c8b581b` had replaced the real catalog dashboard with a ladder stub; restored in `50c6fe0`. (Root cause worth remembering: the suite pins the Inertia component **name** at `AdminDashboardTest:64`, so swapping a page's entire body stayed green. The stub read a `funnel` prop the catalog route never sends, fell through to a hardcoded default, and printed "rung 7 of 7" to a signed-in learner.)

**Gate:** `npm run build` exit 0; full suite **127 tests / 44,748 assertions** green. Verified over real HTTP: `POST /login` → `/passimark/focus` (200, 205 `cert_key` options, `USA-CLOUD` optgroups) → `POST /passimark/focus cissp` → `/passimark/permissions` with `preferences.focus` persisted; returning login → `/` rendering `Passimark/Dashboard` with 207 categories and no ladder-stub text. `ContentCoherenceTest`'s registration redirect was updated to the new first-run destination (enrollment assertions untouched).  �  **Branch:** `feature/sprint-8-1-funnel-stories`  �  **Doc:** `docs/sprint-8-1-funnel-stories.md`
...
**Ladder (as shipped):** `GET /` (guest → Lock) → `GET /passimark/splash` → `GET /passimark/intro` →
`GET /passimark/auth` → `/login` → **`GET`+`POST /passimark/focus`** (real picker over the 205 active
tracks, saved to `users.preferences.focus`) → **`GET`+`POST /passimark/permissions/complete`** (skippable;
sets `preferences.funnel_completed`) → `GET /` (Dashboard, now showing the "Your focus" card).
Rungs 1-4 are pre-login; rungs 5-6 are behind `auth` because they read/write the learner's own record.
