...
- **Sprint 5 — worldwide catalog schema + seeder — done** ([worldwide-catalog-sprint-5-report.md](worldwide-catalog-sprint-5-report.md)): `phase_type` session model (`cert|lesson|phase|domain|mock|final`), `theta_required`, exam `time_minutes`/`is_final`/`irt_enabled`, question `correct_key`, tracks `region`; `WorldwidePassimarkCatalogSeeder` (17 certs, faithful to approved source) + `Uniform205CatalogSeeder` (205-cert JSON [worldwide-205-cert-catalog.json](worldwide-205-cert-catalog.json) → 2,870 sessions / 8,610 exams, spec §4b drift zero); phase-1-first-lesson gating + pass-gated auto-unlock; seed switches `SEED_CATALOG`/`SEED_REGIONS`/`SEED_CERTS`/`SEED_LIMIT`; 28 tests / 6,660 assertions green
