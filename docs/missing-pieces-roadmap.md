...
- real multi-cert content — shipped (proven) in Sprint 4.5: the Hexadigitall CISSP 45-day textbook is fully extracted into a committed bundle and seeded through the existing track/session/exam/question model ([cissp-prep-bundle.md](cissp-prep-bundle.md)). This validates the **bundle-per-cert** flow that v4 generalizes to 205 certs. **Sprint 7.5** upgraded the bundle to full v4 metadata (region `USA-IT-SECURITY`, `phase_type` lesson/mock/final ladder, `theta_required` gating, `irt_enabled` CAT exams with `is_final`, 1.5× adaptive time), closing the disjoint "v4 metadata without questions / real questions without v4 metadata" seeder gap.
